...
defmodule SetPluginsCommandTest do
  ...
  test "validate_execution_environment: not specifying enabled_plugins_file is reported as an error", context do
    assert @command.validate_execution_environment([], Map.delete(Map.merge(context[:opts], %{offline: true, online: false}), :enabled_plugins_file)) ==
  ...
  test "validate_execution_environment: not specifying plugins_dir is reported as an error", context do
    assert @command.validate_execution_environment([], Map.delete(Map.merge(context[:opts], %{offline: true, online: false}), :plugins_dir)) ==
